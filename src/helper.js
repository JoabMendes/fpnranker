...
export async function changeRandomScore(arr, refreshTime) {
    return await new Promise((resolve) => {
        ...
        let timer = setInterval(() => {
            ...
            arr[randKey].showScore = arr[randKey].last_valid_round.coefficient + Math.floor(countup * progress);
            if (progress === 1) {
                clearInterval(timer);
                arr[randKey].last_valid_round.coefficient = arr[randKey].showScore;
                resolve(arr);
            }
        }, refreshTime / 60);
    });
...
export async function bubbleSort(arr) {
    ...
    return await new Promise((resolve) => {
        for (let i = arr.length - 1; i >= 0; i--) {
            for (let j = i - 1; j >= 0; j--) {
                if (arr[i].last_valid_round.coefficient > arr[j].last_valid_round.coefficient) {
                    tmp = arr[j];
                    arr[j] = arr[i];
                    arr[i] = tmp;
                }
